...
function getStudentTimetableDetails(){
    $.ajax({
        method:"GET",
        url:"http://localhost:8080/api/td/timetableData/getStudentTimetableData/"+grade+"/"+classRoom,
        async:true,
        success:function (data){
            if(data.code === "00"){
                ...
                let msg =`
                        <p style="color:white;background-color: red;border: red;border-radius: 10px;padding: 10px font-size: 20px;text-align: center">Timetabe data still not addes to the server!</p>
                    `;
                $('.msg').append(msg);
            }
